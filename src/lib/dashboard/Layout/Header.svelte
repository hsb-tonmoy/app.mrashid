	...
	<div class="flex items-center gap-x-4">
		{#if $page.url.pathname !== '/dashboard'}
			<div
				...
				class="flex justify-center items-center w-14 h-10 bg-[#F8F8F8] hover:bg-gray-300 rounded-2xl cursor-pointer shadow-sm"
			>
				<span class="text-[#757D8A] font-bold w-5 h-5">{@html arrow_left}</span>
			</div>
		{/if}
		<h1 class="text-2xl text-[#404D61] font-bold">{title}</h1>
	</div>
	<div class="flex gap-x-2">
		<div
			on:click={() => (timeline_show = !timeline_show)}
			class={`flex lg:hidden group hover:bg-accent1 justify-center items-center w-14 h-12 border ${
				timeline_show ? 'bg-accent1 border-accent1' : 'border-gray-200 hover:border-accent1'
			} rounded-xl cursor-pointer shadow-sm icons transition-all ease-in-out duration-200`}
		>
			<span
		...
		<div
			class="flex group hover:bg-[#08B295] justify-center items-center w-14 h-12 border border-gray-200 hover:border-[#08B295] rounded-xl cursor-pointer shadow-sm icons transition-all ease-in-out duration-200"
		>
			<span class="text-[#08B295] group-hover:text-white w-8 h-8">{@html question}</span>
		</div>
		<div
