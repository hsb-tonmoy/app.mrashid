...
void (async function main() {
	if (
		...
		if (typeof accessTokenParts.exp !== 'undefined' && accessTokenParts.exp < now) {
		...
		if (typeof refreshTokenParts.exp !== 'undefined' && refreshTokenParts.exp < now) {
...
export const register = async (email, first_name, last_name, password, re_password) => {
	...
	try {
		...
		if (res.ok) {
			...
			console.log(data);
			if (data.email && data.email[0] === 'Account with this Email Address already exists.') {
				register_status.set('email_exists');
			} else if (data.password && data.password[0] === "The two password fields didn't match.") {
				...
				data.password &&
				data.password[0] === 'This password is too short. It must contain at least 8 characters.'
			) {
				register_status.set('password_short');
			} else if (data.password && data.password[0] === 'This password is too common.') {
				register_status.set('password_common');
			} else if (data.password && data.password[0] === 'This password is entirely numeric.') {
